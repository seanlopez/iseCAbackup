...
if __name__ == "__main__":
    logging = open("./output_log.txt", "w", encoding="utf-8")
    print("[Info] Script init")
    logging.write("[Info] Script init\n")
    ...
    while True:
        ...
        if output_list[-1] == "ISE31-01/admin# ":
            ...
            logging.write("[Info] no existing session\n")
            break
        else:
            ...
            logging.write("[Warning] Destroy the existing session and create a new session\n")
    ...
    if "ISE31-01/admin#" in output_list[-1]:
        ...
        logging.write("[Info] start to export the CA file\n")
        ...
        if "Enter encryption-key for export:" in ise_ssh_session.remote_terminal.recv(65535).decode().split("\n")[-2]:
            ...
            logging.write("[Info] exporting start, estimation timing 100s\n")
            time.sleep(100)
            ...
            logging.write("[Warning] Export Fail, please check the status of ISE and 'ise_info.json' file\n")
            ise_ssh_session.remote_terminal.send("\x03\n".encode())
            ise_ssh_session.remote_terminal.send("\n".encode())
            ise_ssh_session.clear_existing_session()
            exit(0)
        ...
        for line in ca_export_result_output:
            if "ISE CA keys export completed successfully" in line:
                ...
                logging.write("[Info] Export Successfully!\n")
                ...
                logging.write("[Warning] Export Fail\n")
                ise_ssh_session.remote_terminal.send("0\n".encode())
                ise_ssh_session.remote_terminal.send("\n".encode())
                ise_ssh_session.remote_terminal.send("exit\n".encode())
                break
        ...
        logging.write("[Warning] Session Status is abnormal, session is terminated\n")
